...
class ModelRunner:
    ...
    def configure_kv_cache_dtype(self):
        ...
        # The bag leaf get_model().kv_cache_dtype stays the RAW user intent;
        # resolved values are per-runner (a draft can resolve a different
        # dtype than the target). Late consumers reach the resolution through
        # this runner / the configurator (constructed after this resolution,
        # receiving kv_cache_dtype_str), never through the process bag.
    ...
    def update_model_fields(
        ...
        *,
        ...
        # Runtime weight update: a genuine config change, recorded on the
        # process bags (get_model().model_path readers, e.g. EPLB, must see
        # the new path) and in the overrides log for /server_info readback.
