...
def declare_load_time_override(source: str, declared: Dict[str, Any]) -> None:
    ...
    weight-resolved dtypes) after publish: it is written to the config bags
    via ``get_context().override`` (namespace readers see it immediately);
    ``server_args`` stays the pristine startup record. Validated against the
    resolvable whitelist first. During a draft build the draft's own bags are
    published (``preserve_config`` scope), so a draft-load declaration lands
    on — and is discarded with — the draft's bags."""
